---
Title: 'Working with the {{cookiecutter.model_shortname}} model'
---

# Model methods and components

The {{cookiecutter.project_name}} plugin helps you preparing or updating several methods of a {{cookiecutter.model_shortname}} model such as static parameters or forcing.
The main interactions are available from the HydroMT Command Line Interface and allow you to configure
HydroMT in order to build or update {{cookiecutter.model_shortname}} models.
...
[configuration](https://deltares.github.io/hydromt/latest/user_guide/model_config.html) (.yml or .ini) file with model methods and options and
a [data sources](https://deltares.github.io/hydromt/latest/user_guide/data_main) (.yml) file should be prepared.

## Model setup methods

An overview of the available {{cookiecutter.model_shortname}} model setup methods
...
The following table provides an overview of which {{cookicutter.model_classname}}
component contains which {{cookiecutter.model_shortname}} in- and output files. The files are read and written with the associated
...
```
    hydromt build {{cookiecutter.project_shortname}} path/to/built_model -r "{'bbox': [xmin, ymin, xmax, ymax]}" -i {{cookiecutter.project_shortname}}_build.ini -d data_sources.yml -vvv
```
...
# Updating a model
To add or change one or more components of an existing {{cookiecutter.project_shortname}} model the `update` method can be used.
...
```
    hydromt update {{cookiecutter.project_shortname}} path/to/model_to_update -o path/to/updated_model -i {{cookiecutter.project_shortname}}_update.ini -d data_sources.yml -vvv
```
