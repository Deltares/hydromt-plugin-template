...
The following table provides an overview of which {{cookiecutter.model_classname}}
...
```
    hydromt build {{cookiecutter.model_shortname}} path/to/built_model -r "{'bbox': [xmin, ymin, xmax, ymax]}" -i {{cookiecutter.model_shortname}}_build.ini -d data_sources.yml -vvv
```
...
# Updating a model
To add or change one or more components of an existing {{cookiecutter.model_shortname}} model the `update` method can be used.
...
```
    hydromt update {{cookiecutter.model_shortname}} path/to/model_to_update -o path/to/updated_model -i {{cookiecutter.model_shortname}}_update.ini -d data_sources.yml -vvv
```
